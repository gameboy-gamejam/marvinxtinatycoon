...
public class GBMarketPopConfirm extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (ingredient != null) {
			...
			((TextView) findViewById(R.id.textName)).setText(ingredient
					.getName());
			((TextView) findViewById(R.id.textPrice)).setText(price + "G");
		}

		updateQuantity(1);
		findViewById(R.id.buttonSpinner).setOnClickListener(
				new OnClickListener() {
		...
		((ImageView) findViewById(R.id.imageIcon)).setImageBitmap(ImageCache
				.getBitmap(this, "ingredient_" + (ingredientId + 1)));
	}

	private void updateQuantity(int i) {
		quantity = i;
		((TextView) findViewById(R.id.buttonSpinner)).setText(String
				.valueOf(quantity));
		((TextView) findViewById(R.id.textTotalAmount))
				.setText((quantity * price) + "G");
	}
}
