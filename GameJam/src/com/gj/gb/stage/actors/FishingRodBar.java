...
public class FishingRodBar {
	...
	private boolean mIsLakeWithFish;// TODO add ng points for rarity per lake 3
									// rarity option
	...
	public void adjustDifficulty(int difficultyValue, int frameDelay) {
		if (difficultyValue == 0) {
			...
			mMovementRate = BAR_WIDTH
					/ (TOTAL_TIME_MS_BAR_REACH_END_EASY / frameDelay);
			...
			mCatchBarWidth = BAR_WIDTH * DISTANCE_CATCH_BAR_RT_NORMAL;
			mMovementRate = BAR_WIDTH
					/ (TOTAL_TIME_MS_BAR_REACH_END_NORMAL / frameDelay);
			...
			mCatchBarWidth = BAR_WIDTH * DISTANCE_CATCH_BAR_RT_HARD;
			mMovementRate = BAR_WIDTH
					/ (TOTAL_TIME_MS_BAR_REACH_END_HARD / frameDelay);
		}
		mMovableDistanceWidth = BAR_WIDTH - mCatchBarWidth;
	}

	public void drawMe(Canvas canvas) {
		if ((mMovementRate > 0 && BAR_START_X + BAR_WIDTH - POINTER_WIDTH <= mRodPointerPosX
				+ mMovementRate)
				|| (mMovementRate < 0 && BAR_START_X >= mRodPointerPosX
						+ mMovementRate)) {
			mMovementRate *= -1;
		}
		mRodPointerPosX += mMovementRate;
		canvas.drawRect(BAR_START_X - 10, 150, BAR_WIDTH + BAR_START_X + 10,
				200, mBarBorder);
		canvas.drawRect(BAR_START_X, 160, BAR_WIDTH + BAR_START_X, 190,
				mFreeBarSpaceBackground);
		canvas.drawRect(mCatchBarPosX, 160, mCatchBarPosX + mCatchBarWidth,
				190, mCatchBarBackground);
		canvas.drawRect(mRodPointerPosX, 160, mRodPointerPosX + POINTER_WIDTH,
				190, mRodPointerBackground);
	}
	...
	public boolean isHit() {
		return StageHelper.isWithinBorders(mCatchBarPosX, mCatchBarPosX
				+ mCatchBarWidth, 10, 20, mRodPointerPosX, 15);
	}
	...
	/**
	 * @param catchBarPosX
	 *            - random value from mMovableDistanceWidth
	 */
	 ...
	 * 
	 * @param rarityOfFish
	 *            - random value from RARITY_OF_FISH_MAX_PT
	 */
