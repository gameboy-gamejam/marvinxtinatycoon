...
public class Carrot {
	...
	private float mRecordedTouchPtY;// records user's activity for this carrot
									// only
	...
	public Carrot(float posX, float posY, float posXRightBorder,
			float posYBottomBorder) {
		mPosX = posX;
		mPosY = posY;
	...
	public void drawMe(Canvas canvas, Resources res, long inGameCurrentTime) {
		if (mCurrentSkin == null) {
			mCurrentSkin = BitmapFactory.decodeResource(res,
					R.drawable.carrot_empty);
			Bitmap.createScaledBitmap(mCurrentSkin, mWidth, mHeight, false);
		} else if (mState == STATE_SEEDED && inGameCurrentTime > mSeedDownTime) {
			mCurrentSkin = BitmapFactory.decodeResource(res,
					R.drawable.carrot_sprout);
			Bitmap.createScaledBitmap(mCurrentSkin, mWidth, mHeight, false);
			mState = STATE_SPROUT;
		} else if (mState == STATE_UNSELECTED
				|| (mState == STATE_SPROUT && inGameCurrentTime > mSproutDownTime)) {
			mCurrentSkin = BitmapFactory.decodeResource(res,
					R.drawable.carrot_ripe);
			Bitmap.createScaledBitmap(mCurrentSkin, mWidth, mHeight, false);
			mState = STATE_RIPE;
		} else if (mState == STATE_RIPE && inGameCurrentTime > mRipeDownTime) {
			mCurrentSkin = BitmapFactory.decodeResource(res,
					R.drawable.carrot_spoiled);
			Bitmap.createScaledBitmap(mCurrentSkin, mWidth, mHeight, false);
			...
			if (!mHarvestDownTimeNoFade && !mHarvestDownTimeHalfFade
					&& !mHarvestDownTimeFullFade) {
				mCurrentSkin = BitmapFactory.decodeResource(res,
						R.drawable.carrot_harvested);
				Bitmap.createScaledBitmap(mCurrentSkin, mWidth, mHeight, false);
				mHarvestDownTimeNoFade = true;
			} else if (mHarvestDownTimeNoFade) {
				mCurrentSkin = BitmapFactory.decodeResource(res,
						R.drawable.carrot_harvest_fade_half);
				Bitmap.createScaledBitmap(mCurrentSkin, mWidth, mHeight, false);
				mHarvestDownTimeHalfFade = true;
				mHarvestDownTimeNoFade = false;
			} else if (mHarvestDownTimeHalfFade) {
				mCurrentSkin = BitmapFactory.decodeResource(res,
						R.drawable.carrot_harvested_fade_full);
				Bitmap.createScaledBitmap(mCurrentSkin, mWidth, mHeight, false);
	...
	public boolean isHit(float touchPosX, float touchPosY) {
		if (mState == STATE_RIPE || mState == STATE_SELECTED
