...
public class CarrotStage extends Stage {
	...
	private SparseArray<Carrot> touchPoints;// TODO may mas maganda pa kaya kesa
											// sa sparse
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (requestCode == GBMiniGameRewardPopop.REQUEST_CODE_REWARD
				|| requestCode == REQUEST_CODE_GAME_START) {
	...
	protected void showReadyInstruction() {
		...
		intent.putExtra(GBGameTipPopUp.KEY_EXTRA_TIP,
				res.getString(R.string.tip_carrot));
	...
	protected void showPointsAndReward() {
		Intent intent = new Intent(CarrotStage.this,
				GBMiniGameRewardPopop.class);
		intent.putExtra(GBMiniGameRewardPopop.KEY_EXTRA_POINTS, mPointsEarned);
		intent.putExtra(GBMiniGameRewardPopop.KEY_EXTRA_CATEGORY,
				IngredientCategory.CROPS);
		startActivityForResult(intent,
				GBMiniGameRewardPopop.REQUEST_CODE_REWARD);
	}
					...
					if (mSurfaceHolder.getSurface().isValid()) {
						...
						synchronized (mSurfaceHolder) {
							if (canvas != null) {
								...
								float currentHeightTimer = INITIAL_HEIGHT_RED_TIMER
										* timerRate;
								canvas.drawRect(653, 50, 900, 100,
										mTimerLabelBackground);
								canvas.drawText(res.getString(R.string.score)
										+ " " + String.valueOf(mPointsEarned),
										703, 80, mTimerLabel);
								canvas.drawRect(703, 89, 853, 593,
										mTimerBackground);
								canvas.drawRect(713, 99 + currentHeightTimer,
										843, 581, mTimer);
								int oddChecker = mSeeder.nextInt(50);
								if (oddChecker % 2 != 0) {// true if seeding
									int soil = mSeeder
											.nextInt(CARROT_NUMBER_OF_SOIL);
		...
		mOnTouchListener = new View.OnTouchListener() {
			...
			public boolean onTouch(View view, MotionEvent event) {
				...
				case MotionEvent.ACTION_POINTER_DOWN:
					...
					if (StageHelper.isWithinBorders(mLeftBorderCanvasPos,
							mRightBorderCanvasPos, mTopBorderCanvasPos,
							mBottomBorderCanvasPos, touchPosX, touchPosY)) {
						for (Carrot carrot : carrots) {
				...
				case MotionEvent.ACTION_MOVE:
					...
					if (carrotOnMove != null) {
						...
						if (distance >= DEFAULT_SWIPE_DISTANCE_PX/*
																 * ||
																 * carrotOnMove
																 * .getState()
																 * == Carrot.
