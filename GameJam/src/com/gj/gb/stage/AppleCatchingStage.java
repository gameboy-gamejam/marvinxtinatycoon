...
public class AppleCatchingStage extends Stage implements SensorEventListener {
						...
						synchronized (mSurfaceHolder) {
							if (canvas != null) {
								...
								if (!tree.isShaking()) {
									...
									if (apples.size() < MAX_APPLE_PER_SCREEN
											&& seed % 6 == 0
											&& inGameCurrentTime <= mEstimatedTimeFinish) {
										apples.add(new Apple(mSeeder
												.nextInt(553) + 50, 50, 500));
									}
									...
									if (apples.size() == 0
										...
										mSensorManager
												.unregisterListener(AppleCatchingStage.this);
	...
	protected void showReadyInstruction() {
		...
		intent.putExtra(GBGameTipPopUp.KEY_EXTRA_TIP,
				res.getString(R.string.tip_fruit));
	...
	protected void showPointsAndReward() {
		Intent intent = new Intent(AppleCatchingStage.this,
